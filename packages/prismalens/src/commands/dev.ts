...
export class DevCommand extends BaseCommand {
	...
	async run(options: DevOptions): Promise<void> {
		...
		const runAll =
			!options.apiOnly && !options.frontendOnly && !options.workerOnly;
		...
		if (runAll || options.apiOnly) {
			console.log(
				chalk.dim("    API:     ") +
					chalk.cyan(`http://${options.host}:${options.port}`),
			);
		}
		if (runAll || options.frontendOnly) {
			console.log(
				chalk.dim("    Frontend: ") +
					chalk.cyan(`http://${options.host}:${options.frontendPort}`),
			);
		}
		if (runAll || options.workerOnly) {
	...
	private async startWorker(): Promise<void> {
		...
		this.workerProcess = spawn(
			"uv",
			["run", "python", "-m", "prismalens_worker"],
			{
				cwd: workerPath,
				env: { ...process.env },
				stdio: "inherit",
