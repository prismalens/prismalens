...
export class StartCommand extends BaseCommand {
  ...
  private async runMigrations(): Promise<void> {
    ...
    try {
      const prismaPath = resolve(this.apiPath, 'node_modules', '.bin', 'prisma');
      const schemaPath = resolve(this.apiPath, 'prisma', 'schema.prisma');

      await new Promise<void>((resolve, reject) => {
        const proc = spawn(prismaPath, ['migrate', 'deploy', '--schema', schemaPath], {
          cwd: this.apiPath,
          env: { ...process.env },
          stdio: 'pipe',
        });
  ...
  private async startApi(host: string, port: number): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      this.apiProcess?.stdout?.on('data', (data) => {
        ...
        if (output.includes('Nest application successfully started') || output.includes('Listening')) {
