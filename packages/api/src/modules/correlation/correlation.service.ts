...
import type { CorrelationRule, Alert, Incident } from '../../../prisma/generated/client.js';
...
export class CorrelationService {
   ...
   */
  async updateRule(id: string, dto: UpdateCorrelationRuleDto): Promise<CorrelationRule | null> {
    try {
  ...
  generateFingerprint(alert: {
    ...
    const normalized = [
      ...
      (alert.description ?? '').toLowerCase().replace(/[^a-z0-9]/g, '').substring(0, 100),
    ].join(':');
   ...
   */
  private async matchToIncidentByRules(alert: Alert): Promise<CorrelationResult> {
  ...
  private alertMatchesRule(alert: Alert, rule: CorrelationRule): boolean {
    try {
      ...
      if (criteria.match.tags && criteria.match.tags.length > 0) {
        const alertTags = alert.tags ? JSON.parse(alert.tags) as string[] : [];
        const hasMatchingTag = criteria.match.tags.some(t => alertTags.includes(t));
   ...
   */
  private async matchToIncidentByFingerprint(alert: Alert): Promise<CorrelationResult> {
   ...
   */
  private async matchToIncidentByTimeWindow(alert: Alert): Promise<CorrelationResult> {
    const timeWindowStart = new Date(Date.now() - 60 * 60 * 1000);
