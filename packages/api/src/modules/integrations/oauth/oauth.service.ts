...
export class OAuthService {
  ...
  async getOAuthConfig(provider: string): Promise<OAuthConfig | null> {
    ...
    try {
      return this.credentialsService.decrypt<OAuthConfig>(definition.oauthConfig);
    } catch {
  ...
  async handleCallback(
    ...
    this.logger.log(`OAuth connection created: ${connection.id} for ${provider}`);
    return connection;
  }
  ...
  private parseStandardTokenResponse(data: Record<string, unknown>): OAuthTokens {
   ...
   */
  async refreshToken(connectionId: string): Promise<IntegrationConnection | null> {
    ...
    const credentials = this.credentialsService.decrypt<OAuthTokens>(connection.credentials);
    if (!credentials.refreshToken) {
      this.logger.warn(`No refresh token available for connection ${connectionId}`);
      return null;
    }
    ...
    try {
      ...
      this.logger.error(`Failed to refresh token for ${connectionId}: ${errorMessage}`);
