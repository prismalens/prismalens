...
export type { IntegrationDefinition, IntegrationConnection, ServiceIntegration };
...
export class IntegrationsService {
  ...
  async findDefinitionByName(name: string): Promise<IntegrationDefinition | null> {
  ...
  async createConnection(dto: CreateConnectionDto): Promise<IntegrationConnection> {
    ...
    if (definition.category === 'monitoring' && definition.maxConnectionsCE === 1) {
  ...
  async findConnectionById(id: string): Promise<IntegrationConnectionWithDefinition | null> {
  ...
  private async testPrometheusConnection(
    ...
    try {
      ...
      return { success: false, error: `Prometheus returned ${response.status}` };
    } catch (error) {
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Connection failed',
   ...
   */
  async getIntegrationsForService(serviceId?: string): Promise<IntegrationContext[]> {
   ...
   */
  async getConnectionWithMaskedCredentials(
    id: string,
  ): Promise<(IntegrationConnectionWithDefinition & { maskedCredentials: Record<string, unknown> }) | null> {
    const connection = await this.findConnectionById(id);
    if (!connection) {
      return null;
    }
