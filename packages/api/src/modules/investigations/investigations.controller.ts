...
export class InvestigationsController {
  ...
  investigations() {
    return {
      ...
      create: implement(investigationsContract.create).handler(async ({ input }) => {
      ...
      list: implement(investigationsContract.list).handler(async ({ input }) => {
        const investigations = await this.investigationsService.findAll({
          status: input.status,
          limit: input.limit,
          offset: input.offset,
        });
        return investigations.map((i) => this.serializeInvestigationWithRelations(i));
      }),

      // GET /investigations/:id - Get a single investigation
      get: implement(investigationsContract.get).handler(async ({ input }) => {
        const investigation = await this.investigationsService.findById(input.id);
        if (!investigation) {
          throw new ORPCError('NOT_FOUND', { message: `Investigation ${input.id} not found` });
        }
      ...
      getStatus: implement(investigationsContract.getStatus).handler(async ({ input }) => {
        const investigation = await this.investigationsService.findById(input.id);
        ...
        return {
          investigation: investigation ? this.serializeInvestigationWithRelations(investigation) : null,
          job: jobStatus
            ? {
                id: jobId,
      ...
      getAgentExecutions: implement(investigationsContract.getAgentExecutions).handler(async ({ input }) => {
        const investigation = await this.investigationsService.findById(input.id);
        if (!investigation) {
          throw new ORPCError('NOT_FOUND', { message: `Investigation ${input.id} not found` });
        }

        const executions = await this.investigationsService.getAgentExecutions(input.id);
        return executions.map((e) => this.serializeAgentExecution(e));
      }),

      // POST /investigations/:id/cancel - Cancel an investigation
      cancel: implement(investigationsContract.cancel).handler(async ({ input }) => {
        ...
        const updated = await this.investigationsService.updateStatus(input.id, 'cancelled');
        return this.serializeInvestigation(updated || investigation);
      }),

      // PATCH /investigations/:id/status - Update status (Worker)
      updateStatus: implement(investigationsContract.updateStatus).handler(async ({ input }) => {
        const investigation = await this.investigationsService.findById(input.id);
        if (!investigation) {
          throw new ORPCError('NOT_FOUND', { message: `Investigation ${input.id} not found` });
        }
        const updated = await this.investigationsService.updateStatusInternal(input.id, input.status, undefined, input.error);
        return this.serializeInvestigation(updated || investigation);
      }),

      // POST /investigations/:id/result - Write result (Worker)
      writeResult: implement(investigationsContract.writeResult).handler(async ({ input }) => {
        const investigation = await this.investigationsService.findById(input.id);
        if (!investigation) {
          throw new ORPCError('NOT_FOUND', { message: `Investigation ${input.id} not found` });
        }
        ...
        const updated = await this.investigationsService.writeResultWithRelations(input.id, resultDto);
        return this.serializeInvestigationWithRelations(updated || investigation);
      }),
    };
  }
  ...
  private serializeInvestigationWithRelations(investigation: any): any {
    ...
    if (investigation.recommendations) {
      serialized.recommendations = investigation.recommendations.map((r: any) => ({
        id: r.id,
        title: r.title,
        priority: r.priority,
        status: r.status,
