...
export class RecommendationsController {
	...
	recommendations() {
		return {
			...
			list: implement(recommendationsContract.list).handler(
				async ({ input }) => {
					...
					return recommendations.map((r) =>
						this.serializeRecommendationWithRelations(r),
					);
				},
			),

			// GET /recommendations/stats - Get recommendation statistics
			getStats: implement(recommendationsContract.getStats).handler(
				async () => {
			...
			get: implement(recommendationsContract.get).handler(async ({ input }) => {
				...
				if (!recommendation) {
					throw new ORPCError("NOT_FOUND", {
						message: `Recommendation ${input.id} not found`,
					});
				}
			...
			update: implement(recommendationsContract.update).handler(
				async ({ input }) => {
					const { id, ...updateData } = input;
					const recommendation = await this.recommendationsService.update(
						id,
						updateData as UpdateRecommendationDto,
					);
					if (!recommendation) {
						throw new ORPCError("NOT_FOUND", {
			...
			complete: implement(recommendationsContract.complete).handler(
				async ({ input }) => {
					const recommendation = await this.recommendationsService.complete(
						input.id,
					);
					if (!recommendation) {
						throw new ORPCError("NOT_FOUND", {
			...
			dismiss: implement(recommendationsContract.dismiss).handler(
				async ({ input }) => {
					const recommendation = await this.recommendationsService.dismiss(
						input.id,
					);
					if (!recommendation) {
						throw new ORPCError("NOT_FOUND", {
