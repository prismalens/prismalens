...
describe('ServiceDiscoveryController (BDD)', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        { provide: ServiceDiscoveryService, useValue: mockServiceDiscoveryService },
      ],
    }).compile();
  ...
  describe('triggerDiscovery', () => {
    it('should trigger discovery for connection', async () => {
      ...
      mockServiceDiscoveryService.discoverFromConnection.mockResolvedValue(suggestions);
  ...
  describe('getPendingSuggestions', () => {
    it('should return all pending suggestions', async () => {
      ...
      mockServiceDiscoveryService.getPendingSuggestions.mockResolvedValue(suggestions);
  ...
  describe('acceptSuggestion', () => {
    it('should accept suggestion without overrides', async () => {
      ...
      mockServiceDiscoveryService.acceptSuggestion.mockResolvedValue(createdService);

      const result = await controller.acceptSuggestion(suggestionId);

      expect(result).toEqual(createdService);
      expect(service.acceptSuggestion).toHaveBeenCalledWith(suggestionId, undefined);
    });
    ...
    it('should accept suggestion with overrides', async () => {
      ...
      expect(result).toEqual(createdService);
      expect(service.acceptSuggestion).toHaveBeenCalledWith(suggestionId, overrides);
    });
  ...
  describe('acceptBulk', () => {
    it('should accept multiple suggestions', async () => {
      ...
      expect(service.acceptMultiple).toHaveBeenCalledWith(dto.suggestionIds, dto.overrides);
    });
    ...
    it('should accept bulk with partial services', async () => {
      ...
      expect(service.acceptMultiple).toHaveBeenCalledWith(['sugg-1', 'sugg-2', 'sugg-3'], undefined);
    });
  });
});
