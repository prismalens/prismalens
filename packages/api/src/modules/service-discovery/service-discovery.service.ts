...
export class ServiceDiscoveryService {
   ...
   */
  async discoverFromConnection(connectionId: string): Promise<ServiceSuggestion[]> {
  ...
  async discoverFromGitHub(
    ...
    this.logger.log(`Discovering services from GitHub connection: ${connection.name}`);
    ...
    for (const repo of repositories) {
      try {
        ...
        const { isMonorepo, services } = await this.analyzeRepoStructure(repo, connection);

        for (const service of services) {
          // Determine subPath value (null for root, string for subdirectories)
          const subPath: string | null = service.path === '.' ? null : service.path;
          ...
          if (existing && existing.status !== 'rejected' && existing.status !== 'ignored') {
  ...
  async acceptSuggestion(
    ...
    const service = await this.prisma.service.create({
      data: {
        name: serviceName,
        displayName: overrides?.displayName || suggestion.displayName || serviceName,
    ...
    this.logger.log(`Accepted service suggestion: ${serviceName} (${service.id})`);
