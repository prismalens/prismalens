...
export class ServiceDiscoveryController {
  ...
  serviceDiscovery() {
    return {
      ...
      listSuggestions: implement(serviceDiscoveryContract.listSuggestions).handler(async () => {
        const suggestions = await this.serviceDiscoveryService.getPendingSuggestions();
        return suggestions.map((s) => this.serializeSuggestion(s));
      }),

      // GET /service-discovery/suggestions/:id - Get a single suggestion
      getSuggestion: implement(serviceDiscoveryContract.getSuggestion).handler(async ({ input }) => {
        // Use findById pattern since getSuggestionById doesn't exist
        const suggestions = await this.serviceDiscoveryService.getPendingSuggestions();
        const suggestion = suggestions.find((s) => s.id === input.id);
        if (!suggestion) {
          throw new ORPCError('NOT_FOUND', { message: `Suggestion ${input.id} not found` });
        }
        return this.serializeSuggestion(suggestion);
      }),

      // POST /service-discovery/suggestions/:id/accept - Accept a suggestion
      acceptSuggestion: implement(serviceDiscoveryContract.acceptSuggestion).handler(async ({ input }) => {
        const { id, ...overrides } = input;
        this.logger.log(`Accepting service suggestion: ${id}`);
        const service = await this.serviceDiscoveryService.acceptSuggestion(id, overrides);
        return {
      ...
      rejectSuggestion: implement(serviceDiscoveryContract.rejectSuggestion).handler(async ({ input }) => {
      ...
      ignoreSuggestion: implement(serviceDiscoveryContract.ignoreSuggestion).handler(async ({ input }) => {
      ...
      acceptBulkSuggestions: implement(serviceDiscoveryContract.acceptBulkSuggestions).handler(async ({ input }) => {
        this.logger.log(`Accepting ${input.suggestionIds.length} service suggestions`);
        const services = await this.serviceDiscoveryService.acceptMultiple(input.suggestionIds);
        return {
      ...
      triggerDiscovery: implement(serviceDiscoveryContract.triggerDiscovery).handler(async ({ input }) => {
        this.logger.log(`Triggering service discovery for connection: ${input.connectionId}`);
        const suggestions = await this.serviceDiscoveryService.discoverFromConnection(input.connectionId);
        return {
