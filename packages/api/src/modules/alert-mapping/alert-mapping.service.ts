...
export class AlertMappingService {
  ...
  async create(dto: CreateMappingRuleDto): Promise<AlertMappingRule> {
    const matchCriteria =
      typeof dto.matchCriteria === 'string' ? dto.matchCriteria : JSON.stringify(dto.matchCriteria);
  ...
  async update(id: string, dto: UpdateMappingRuleDto): Promise<AlertMappingRule> {
  ...
  private matchesRule(alert: AlertInfo, rule: AlertMappingRule): boolean {
    try {
      const criteria = JSON.parse(rule.matchCriteria) as Record<string, unknown>;
      ...
      this.logger.error(`Error parsing rule criteria for rule ${rule.id}`, error);
      return false;
    }
  }
