...
import { AlertMappingService, AlertInfo } from './alert-mapping.service.js';
...
describe('AlertMappingService (BDD)', () => {
  ...
  describe('findAll', () => {
    ...
    it('should only return enabled rules', async () => {
      ...
      mockPrismaService.alertMappingRule.findMany.mockResolvedValue([enabledRule]);
  ...
  describe('findById', () => {
    it('should return rule when found', async () => {
      ...
      mockPrismaService.alertMappingRule.findUnique.mockResolvedValue(expectedRule);

      const result = await service.findById(ruleId);

      expect(result).toEqual(expectedRule);
      expect(mockPrismaService.alertMappingRule.findUnique).toHaveBeenCalledWith({
        where: { id: ruleId },
      });
    });
  ...
  describe('resolveServiceForAlert', () => {
    it('should return service for matching alert', async () => {
      ...
      const rule = {
        ...
        enabled: true,
        priority: 1,
        matchCriteria: JSON.stringify({ source: 'prometheus', labels: { env: 'prod' } }),
        serviceId: 'service-1',
        description: null,
        createdAt: new Date(),
        updatedAt: new Date(),
