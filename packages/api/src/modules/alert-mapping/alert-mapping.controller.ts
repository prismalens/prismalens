...
import type { CreateMappingRuleDto, UpdateMappingRuleDto } from './dto/index.js';
...
export class AlertMappingController {
  ...
  alertMapping() {
    return {
      ...
      create: implement(alertMappingContract.create).handler(async ({ input }) => {
        this.logger.log(`Creating alert mapping rule: ${input.name}`);
        const rule = await this.alertMappingService.create(input as CreateMappingRuleDto);
      ...
      get: implement(alertMappingContract.get).handler(async ({ input }) => {
        ...
        if (!rule) {
          throw new ORPCError('NOT_FOUND', { message: `Alert mapping rule ${input.id} not found` });
        }
        return this.serializeRuleWithService(rule);
      }),

      // PATCH /alert-mapping/rules/:id - Update an alert mapping rule
      update: implement(alertMappingContract.update).handler(async ({ input }) => {
        const { id, ...updateData } = input;
        this.logger.log(`Updating alert mapping rule: ${id}`);
        const rule = await this.alertMappingService.update(id, updateData as UpdateMappingRuleDto);
        if (!rule) {
          throw new ORPCError('NOT_FOUND', { message: `Alert mapping rule ${id} not found` });
        }
        return this.serializeRule(rule);
      }),

      // DELETE /alert-mapping/rules/:id - Delete an alert mapping rule
      delete: implement(alertMappingContract.delete).handler(async ({ input }) => {
      ...
      test: implement(alertMappingContract.test).handler(async ({ input }) => {
        ...
        this.logger.log(`Testing mapping for alert: "${alertData.title || 'unknown'}"`);
