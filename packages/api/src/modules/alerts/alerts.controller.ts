...
export class AlertsController {
  ...
  alerts() {
    return {
      ...
      listUncorrelated: implement(alertsContract.listUncorrelated).handler(async ({ input }) => {
      ...
      get: implement(alertsContract.get).handler(async ({ input }) => {
        ...
        if (!alert) {
          throw new ORPCError('NOT_FOUND', { message: `Alert ${input.id} not found` });
        }
      ...
      update: implement(alertsContract.update).handler(async ({ input }) => {
        ...
        const alert = await this.alertsService.update(id, updateData as UpdateAlertDto);
        if (!alert) {
      ...
      acknowledge: implement(alertsContract.acknowledge).handler(async ({ input }) => {
      ...
      correlate: implement(alertsContract.correlate).handler(async ({ input }) => {
        const alert = await this.alertsService.findById(input.id);
        if (!alert) {
          throw new ORPCError('NOT_FOUND', { message: `Alert ${input.id} not found` });
        }
  ...
  private serializeAlertWithRelations(alert: any): any {
    ...
    if (alert.service) {
      serialized.service = {
        ...
        metadata: alert.service.metadata ? JSON.parse(alert.service.metadata) : null,
