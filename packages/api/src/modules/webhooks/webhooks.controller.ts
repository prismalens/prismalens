...
export class WebhooksController {
  ...
  webhooks() {
    return {
      ...
      generic: implement(webhooksContract.generic).handler(async ({ input }) => {
        this.logger.log('Received generic webhook');

        const result = await this.webhooksService.processGenericWebhook(input as unknown as GenericWebhookDto);

        return this.formatResponse(result);
      }),

      // POST /webhooks/prometheus - Receive Prometheus AlertManager webhook
      prometheus: implement(webhooksContract.prometheus).handler(async ({ input }) => {
        this.logger.log(`Received Prometheus webhook with ${input.alerts?.length ?? 0} alerts`);
        ...
        for (const alert of input.alerts ?? []) {
          try {
            const genericDto: GenericWebhookDto = {
              ...
              severity: this.mapPrometheusLabelToSeverity(alert.labels?.severity),
              source: 'prometheus',
              labels: alert.labels,
              sourceEventId: alert.fingerprint,
            };
            const result = await this.webhooksService.processGenericWebhook(genericDto);
  ...
  private mapPrometheusLabelToSeverity(severityLabel?: string): Severity | undefined {
