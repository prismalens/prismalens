...
async function bootstrap() {
  ...
  if (protocol === 'https') {
    ...
    if (keyPath && certPath) {
      if (fs.existsSync(keyPath) && fs.existsSync(certPath)) {
        ...
        logger.error(`SSL Key or Cert file not found. Key: ${keyPath}, Cert: ${certPath}`);
        process.exit(1);
      }
    } else {
      logger.error('PRISMALENS_PROTOCOL is https but PRISMALENS_SSL_KEY or PRISMALENS_SSL_CERT is missing.');
      process.exit(1);
    }
  }
  ...
  const dashboardUrl = configService.get<string>('PRISMALENS_DASHBOARD_BASE_URL');
  ...
  logger.log(`API documentation: ${protocolDisplay}://${host}:${port}/api/docs`);
}
