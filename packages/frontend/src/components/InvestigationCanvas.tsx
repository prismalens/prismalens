...
import { AlertCircle, Search, Brain, Lightbulb, CheckCircle, Loader2 } from 'lucide-react'
...
export default function InvestigationCanvas({ investigationId, data }: InvestigationCanvasProps) {
  const initialNodes: Node[] = useMemo(() => [
    ...
    },
    {
      ...
      data: {
        label: data?.gathererData ? 'Context collected' : 'Collecting context...',
        status: data?.gathererData ? 'completed' : data?.alertData ? 'running' : 'pending',
      },
    },
    {
      id: 'analyzer',
      ...
      data: {
        ...
        status: data?.analyzerData ? 'completed' : data?.gathererData ? 'running' : 'pending',
    ...
    },
    {
      ...
      data: {
        label: data?.recommenderData ? 'Recommendations ready' : 'Generating recommendations...',
        status: data?.recommenderData ? 'completed' : data?.analyzerData ? 'running' : 'pending',
