// Native Gemini backend — calls the v1beta generateContent REST endpoint directly.
// The API key goes in the `x-goog-api-key` HEADER (never the URL); error bodies are
// sanitized. Kept as a tested reference adapter; most providers go through the universal
...
// unlike Vertex AI, there is no "function" role. Tool results ride on a "user" turn.
function toGeminiContent(m: Message): GeminiContent {
	...
	for (const tr of m.toolResults ?? []) {
		parts.push({ functionResponse: { name: tr.name, response: tr.isError ? { error: tr.output } : { result: tr.output } } });
	}
