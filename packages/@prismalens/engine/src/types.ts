// Core types for the prismalens investigation engine.
//
// The LOOP owns orchestration (ADR-0003). Its only model dependency is a
// `ModelBackend` (ADR-0004); tools are plain functions in a registry. The report uses
// ordered hypotheses + evidence status, with NO numeric confidence (ADR-0002). The loop
// emits `StepEvent`s as an async stream for the live drill-down UI (ADR-0007).
...
export interface ToolCall {
	/** Id matching a call to its result. Some backends (OpenAI) require it; the loop threads it. */
	id: string;
	name: string;
...
/** A JSON-schema subset both OpenAI-compatible and Gemini APIs accept for tool params. */
...
export interface ModelBackend {
	readonly id: string;
	complete(system: string, messages: Message[], tools: ToolDef[]): Promise<ModelResponse>;
}
...
// ---- Loop telemetry — the live stream the UI consumes (ADR-0007) ----
