...
export const ServiceTierSchema = z.enum(['tier_1', 'tier_2', 'tier_3', 'tier_4'])

export const DependencyTypeSchema = z.enum(['runtime', 'build', 'data'])

export const DependencyCriticalitySchema = z.enum(['required', 'optional', 'degraded'])
...
export const ToolCategorySchema = z.enum(['file', 'search', 'github', 'logs', 'analysis'])
...
// Postmortem
export const PostmortemStatusSchema = z.enum(['draft', 'in_review', 'published', 'archived'])

// Correlation
export const CorrelationActionSchema = z.enum(['correlate', 'suppress', 'create_incident'])

// Settings
export const SettingTypeSchema = z.enum(['string', 'number', 'boolean', 'json'])

export const SettingCategorySchema = z.enum(['general', 'correlation', 'ai', 'notifications'])

// Integrations
export const ConnectionStatusSchema = z.enum(['pending', 'connected', 'error', 'disabled'])

export const AuthMethodSchema = z.enum(['api_key', 'oauth2'])

// Service Discovery
export const SuggestionStatusSchema = z.enum(['pending', 'accepted', 'rejected', 'ignored'])
