...
const CodeChangeInputSchema = z.object({
    filePath: z
        .string()
        .describe('Full path to the file from repository root (e.g., "src/services/auth.ts")'),
    searchBlock: z
        .string()
        .describe(
    ...
    replaceBlock: z
        .string()
        .describe('New content to replace with - use proper formatting and indentation'),
    testCase: z
        .string()
        .describe(
...
const RecommendationInputSchema = z.object({
    title: z
        .string()
        .max(100)
        .describe('Clear, actionable title (e.g., "Fix null pointer in auth handler")'),
    description: z
        .string()
        .optional()
        .describe('Detailed explanation of the fix and why it addresses the root cause'),
    priority: z
    ...
    category: z
        .enum(['code_fix', 'config_change', 'rollback', 'monitoring', 'investigation'])
        .describe(
    ...
    return tool(
        async (input) => {
            try {
                ...
                if (recommendation.codeChanges && recommendation.codeChanges.length > 0) {
...
export function createValidateCodeChangeTool(): StructuredTool {
    return tool(
        async ({ recommendationIndex, codeChangeIndex, searchFound, syntaxValid, notes }) => {
            if (recommendationIndex < 0 || recommendationIndex >= recommendationStore.length) {
                return JSON.stringify({
                    status: 'error',
            ...
            const validationStatus = searchFound && syntaxValid ? 'VALIDATED' : 'NEEDS_REVIEW';
        ...
        },
        {
            ...
            schema: z.object({
                ...
                codeChangeIndex: z
                    .number()
                    .describe('Index of the code change within the recommendation (0-based)'),
                searchFound: z
                    .boolean()
                ...
                notes: z
                    .string()
                    .optional()
                    .describe('Any notes about the validation'),
            }),
        }
    );
}
