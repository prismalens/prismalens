...
/**
 * Get GitHub token from bundle context
 */
function getGitHubTokenFromContext(
	context: BundleExecutionContext,
): string | undefined {
	return getGitHubToken(context.integrations);
}
...
/**
 * Create GitHub code tools from bundle context.
 * Used by the progressive disclosure system.
 */
export function createGitHubCodeBundle(
	context: BundleExecutionContext,
): StructuredTool[] {
	const token = getGitHubTokenFromContext(context);
	return createGitHubToolsInternal(token);
}

/**
 * Internal function to create tools with a token.
 * Shared between legacy and bundle factory paths.
 */
function createGitHubToolsInternal(token: string | undefined): StructuredTool[] {
	return [
		// Get file contents from a repository
		tool(
			async ({ owner, repo, path, ref }) => {
				try {
					...
					return `Error fetching file: ${axiosError.message || "Unknown error"}`;
				}
			},
			{
				name: "github_get_file",
		...
		tool(
			async ({ owner, repo, query, extension }) => {
				try {
					...
					const items = response.data.items.slice(0, 10).map((item: Record<string, unknown>) => ({
						path: item.path,
						name: item.name,
						htmlUrl: item.html_url,
					}));
					...
					return `Error searching code: ${axiosError.message || "Unknown error"}`;
				}
			},
			{
				name: "github_search_code",
		...
		tool(
			async ({ owner, repo, path, ref }) => {
				try {
					...
					const items = response.data.map((item: Record<string, unknown>) => ({
						name: item.name,
						type: item.type,
						path: item.path,
						size: item.size,
					...
					return `Error listing directory: ${axiosError.message || "Unknown error"}`;
				}
			},
			{
				name: "github_list_directory",
		...
		tool(
			async ({ owner, repo, path, since, per_page }) => {
				try {
					...
					const commits = response.data.map((commit: Record<string, unknown>) => {
					...
					const axiosError = error as { message?: string };
					return `Error fetching commits: ${axiosError.message || "Unknown error"}`;
				}
			},
			{
				name: "github_get_commits",
