...
// vi.hoisted ensures the counter is available when vi.mock is hoisted.
...
describe("full investigation graph", () => {
  ...
  it("scout produces a self-assessment for analyst", async () => {
    ...
    counter.value = 1 // Skip to "end" response
  ...
  it("iteration budget prevents infinite loops", async () => {
    ...
    const { createLLM, resolveAgentLLM } = await import("../../llm/factory.js")
    const alwaysGathererLLM = {
      withStructuredOutput: vi.fn(() => ({
        invoke: vi.fn(async () => ({
          agent: "gatherer",
    ...
    vi.mocked(createLLM).mockReturnValue(alwaysGathererLLM)
    vi.mocked(resolveAgentLLM).mockReturnValue(alwaysGathererLLM)
