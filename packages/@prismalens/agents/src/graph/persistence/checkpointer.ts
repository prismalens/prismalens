 ...
 */
export async function getCheckpointer(config?: CheckpointerConfig): Promise<PostgresSaver> {
    ...
    try {
        ...
        poolInstance = null;
        throw new Error(`Failed to connect to PostgreSQL for checkpointing: ${error}`);
    }
 ...
 * }
 */
export async function hasExistingCheckpoint(investigationId: string): Promise<boolean> {
    const checkpointer = await getCheckpointer();
