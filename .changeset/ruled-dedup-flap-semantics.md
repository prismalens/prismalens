...
- **`Alert.dedupKey` is no longer a unique column**, which the new-episode branch requires. Additive migration `20260812180006_alert_dedup_key_not_unique` swaps the unique index for a plain one in both the sqlite and pg lineages; reads take the newest episode.
